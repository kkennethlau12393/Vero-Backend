...
def synthesize_gaps_with_llm(
    ...
    """
    Use LLM to evaluate, filter, and synthesize gap candidates.

    The LLM acts as both a quality gate and a describer:
    1. Evaluates each candidate for genuine research significance
    2. Rejects heuristic artifacts and nonsensical candidates
    3. Describes accepted candidates with evidence-based descriptions

    Returns only accepted gaps (status == "accepted").
    """
    client = get_groq_client()

    prompt = build_synthesis_prompt(candidates_by_type, paper_data)
    ...
    for attempt in range(GROQ_MAX_RETRIES):
        try:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "You are a critical research gap analyst. "
                            "Evaluate candidates honestly -- reject noise, "
                            "describe only genuine gaps. Return only valid JSON."
                        ),
                    },
            ...
            all_gaps = extract_json_from_llm(content, expect_array=True)

            if not isinstance(all_gaps, list):
                logger.warning("LLM returned non-list response")
                return []

            # Filter to only accepted gaps
            accepted = []
            rejected_count = 0
            for gap in all_gaps:
                status = gap.get("status", "accepted")
                if status == "rejected":
                    rejected_count += 1
                    logger.info(
                        f"LLM rejected gap: {gap.get('type', '?')} - "
                        f"{gap.get('rejection_reason', 'no reason')}"
                    )
                else:
                    accepted.append(gap)
            ...
            total = len(all_gaps)
            logger.info(
                f"LLM evaluation: {len(accepted)}/{total} accepted, "
                f"{rejected_count}/{total} rejected"
            )
            return accepted
            ...
            if attempt < GROQ_MAX_RETRIES - 1:
                ...
                logger.warning(
                    f"Groq API error (attempt {attempt + 1}/{GROQ_MAX_RETRIES}), "
                    f"retrying in {wait:.1f}s: {e}"
                )
                time.sleep(wait)
            else:
                logger.exception("Error in LLM synthesis after all retries")
                return []
